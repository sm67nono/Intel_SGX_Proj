...
int initialize_enclave(void)
{
    ...
    ret = sgx_create_enclave(ARITH_CORE_FILENAME, SGX_DEBUG_FLAG, &token, &updated, &global_eid, NULL);
...
/* OCall functions */
void ocall_Arith_Core_sample(const char *str)
{
...
void INIT_ENCLAVE(char* path)
{
	...
	ptr = realpath(dirname(path),absPath);
	if(chdir(absPath)!=0)
		abort();
...
//Arithmetic Evaluation ecall to sgx with input string : manna Dec 2016
JNIEXPORT void JNICALL Java_JavaApp_showResult(JNIEnv *env, jobject obj, jstring str)
{


	 char *path = (char*)malloc(1024);
    //strcpy(path,"/home/neo/eclipse/DSP_LAB/SGX_Evaluator/sgx/enclave_Arith_Core/untrusted");
	 char currentDir[1024];
	 	 if (getcwd(currentDir, sizeof(currentDir)) != NULL)
	 	 {
	 		 strcat(currentDir,"/untrusted");
	 		 strcpy(path,currentDir);
	 		 printf("Path of current Directory is : %s \n", path);
	 	 }
	     else
	 	{
	     	printf("Couldn't access current working directory aborting...");
	     	abort();
	 	}
    INIT_ENCLAVE(path);
    sgx_status_t ret = SGX_ERROR_UNEXPECTED;
    ...
    char* xC = const_cast<char*>((*env).GetStringUTFChars(str,0));
    char result[128];
    ret = ecall_Arith_Core(global_eid, &ecall_return, xC , result,128);
    //ret = ecall_Arith_Core(global_eid, &ecall_return);
    if (ret != SGX_SUCCESS)
            abort();
...
/* Application entry */
int SGX_CDECL not_main(int argc, char *argv[])
{
    (void)(argc);
    (void)(argv);
    ...
    else
	{
     ...
     char* xC="3+8-2";
     ret = ecall_Arith_Core(global_eid, &ecall_return, xC , result,128);
     //ret = ecall_Arith_Core_sample(global_eid, &ecall_return);
     if (ret != SGX_SUCCESS)
             abort();
...
JNIEXPORT jbyteArray JNICALL Java_JavaSGX_sgx_1add_1enc(JNIEnv *env, jobject obj, jbyteArray xAr,
	...
	char buffer[xSize + MACLEN + MACLEN];
...
//Code added for library testing
...
void test_enc_add() {
	...
	strcpy(ptxt1, "3+4*19-8+56+81*2");

	//Description
	char ptxt2[ptxt_size];
	strcpy(ptxt2, "Dummy Financial Statement");
